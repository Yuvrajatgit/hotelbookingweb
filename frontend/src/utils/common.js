export const sliderSettings = {
    slidesPerView: 1,
    spaceBetween: 50,
    breakpoints: {
        480: {
            slidesPerView: 1,
        },
        600: {
            slidesPerView: 2,
        },
        750: {
            slidesPerView: 3,
        },
        1100: {
            slidesPerView: 4,
        }
    }
};

export const updateFavourites = (id, favourites)=>{
    // if(favourites?.includes(id)){
    //     return favourites.filter((resId)=> resId!== id)
    // }
    // else {
    //     return [...favourites, id]
    // }
    const updatedFavourites = Array.isArray(favourites) ? [...favourites] : [];

    if (updatedFavourites.includes(id)) {
      // If id is already in favourites, remove it
      return updatedFavourites.filter((resId) => resId !== id);
    } else {
      // If id is not in favourites, add it
      return [...updatedFavourites, id];
    }
};

export const checkFavourites = (id, favourites)=>{
    // return favourites?.includes(id)? "#fa3e5f" : 'white'
    const isFavorite = Array.isArray(favourites) && favourites.includes(id);

    return isFavorite ? "#fa3e5f" : "white";
  
};

export const validateString = (value)=>{
    return value?.length < 3 || value === null ? "Must have atleast 3 characters" : null;
};